...
Status RotaryEmbeddingOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder, const Node& node,
  ...
  // When positionIds is a 1D tensor, it represents the start offset for each sequence.
  ...
  // First ensure input has shape (batch_size, num_heads, sequence_length, head_size).
  if (!input_is_4d) {
  ...
  // First reshape the partial input0.
  const std::vector<uint32_t> new_partial_input0_shape = interleaved
      ? std::vector<uint32_t>({batch_size, sequence_length, num_heads, rotary_embedding_dim / 2, 2})
  ...
  split_partial_input0_options.set("axis", interleaved ? 4 : 3);
  ...
  emscripten::val concated_partial_input0 = wnn_builder.call<emscripten::val>(
      "concat", split_partial_input0, interleaved ? 4 : 3, concat_partial_input0_options);
  ...
  // Reshape cos_cache and sin_cache with leading 1.
  std::vector<uint32_t> cos_sin_shape = GetVecUint32FromVecInt64(cos_cache_shape);
  cos_sin_shape.insert(cos_sin_shape.begin(), 1);
  emscripten::val reshape_cos_sin_cache_options = emscripten::val::object();
  reshape_cos_sin_cache_options.set("label", node_name + "_reshape_cos_sin_cache");
  cos_cache = wnn_builder.call<emscripten::val>(
      "reshape", cos_cache, emscripten::val::array(cos_sin_shape), reshape_cos_sin_cache_options);
  sin_cache = wnn_builder.call<emscripten::val>(
      "reshape", sin_cache, emscripten::val::array(cos_sin_shape), reshape_cos_sin_cache_options);
  ...
  gather_cos_sin_options.set("axis", 1);
  ...
  const std::vector<uint32_t> reshaped_cos_sin_shape = interleaved
      ? std::vector<uint32_t>({batch_size, sequence_length, rotary_embedding_dim / 2, 1})
      : std::vector<uint32_t>({batch_size, sequence_length, 1, rotary_embedding_dim / 2});
  ...
  emscripten::val sign_constant = emscripten::val::undefined();
  ...
  sign_constant = wnn_builder.call<emscripten::val>("constant", sign_constant_desc, sign_buffer);
  ...
  // Reshape mul_cos and mul_sin to (batchSize, sequenceLength, numHeads, rotaryEmbeddingDim).
