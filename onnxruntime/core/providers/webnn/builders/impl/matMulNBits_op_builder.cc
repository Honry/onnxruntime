...
bool MatMulNBitsBuilder::IsOpSupportedImpl(const InitializedTensorSet& initializers,
                                           const Node& node,
  ...
  if (!Contains(initializers, input_defs[1]->Name())) {  // B
    LOGS(logger, VERBOSE) << "Input B of MatMulNBits [" << name << "] must be known as initializer";
    return false;
  }
  if (TensorExists(input_defs, 3) && !Contains(initializers, input_defs[3]->Name())) {  // zero_points
...
bool MatMulNBitsBuilder::HasSupportedInputsImpl(const InitializedTensorSet& /* initializers */,
