...
Status GatherBlockQuantizedOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder,
  ...
  std::vector<uint32_t> zero_points_shape;
  ...
  if (TensorExists(input_defs, 3)) {  // zero_points
    ...
    const uint8_t default_zero_point =
        bits == 4 ? input_type == ONNX_NAMESPACE::TensorProto_DataType_INT4 ? 0 : 8 : 128;
...
bool GatherBlockQuantizedOpBuilder::HasSupportedInputsImpl(const GraphViewer&, const Node& node,
  ...
  // dequantizeLinear's input should be same as input
  // gather's input should be same as scales
...
bool GatherBlockQuantizedOpBuilder::HasSupportedOutputsImpl(const Node& node,
  ...
  // Only need to check the output data type of ops that produce the outputs of GatherBlockQuantized.
  // gather's output should be same as output
