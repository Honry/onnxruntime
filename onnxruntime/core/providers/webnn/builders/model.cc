...
Model::Model(const emscripten::val& context, const emscripten::val& graph, const logging::Logger& logger, bool use_dispatch)
    : wnn_context_(context),
      ...
      supports_compute_shapes_(IsComputeShapesSupported()) {}

Model::~Model() {}

Status Model::ComputeShapes(
  ...
  for (const auto& [name, shape] : input_shapes) {
    ...
    for (const auto& dim : shape) {
      js_shape.call<void>("push", SafeInt<uint32_t>(dim).Ref());
    }
    js_input_shapes.set(name, js_shape);
  }

  emscripten::val result = wnn_graph_.call<emscripten::val>("computeShapes", js_input_shapes).await();

  if (result.isUndefined() || result.isNull()) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "MLGraph.computeShapes() returned undefined/null");
  }

  auto object = emscripten::val::global("Object");
  emscripten::val keys = object.call<emscripten::val>("keys", result);
  const uint32_t num_outputs = keys["length"].as<uint32_t>();
  ...
  for (uint32_t i = 0; i < num_outputs; ++i) {
    ...
    for (uint32_t d = 0; d < rank; ++d) {
      shape.push_back(static_cast<int64_t>(js_shape[d].as<uint32_t>()));
    }
